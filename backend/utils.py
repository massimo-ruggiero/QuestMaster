...
def get_llm(model: str = "gpt-4.1-nano"):
    if "gpt" in model:
        ...
        return ChatGoogleGenerativeAI(
            ...
            temperature=0,
            max_retries=3,
            api_key=config.get('GEMINI_API_KEY_C', "")
        )
    else:
        raise ValueError(f"Unsupported model: {model}")

def get_image_model(model: str = "dall-e-3", size: str ="1792x1024", quality: str ="standard"):
    return DallEAPIWrapper(api_key= config.get('OPENAI_API_KEY_C', ""), 
                           model= model, 
                           size= size, 
                           quality= quality)


def fill_generative_system_template(template: str, examples_path: str = "examples") -> str:
...
def fill_retrieval_system_template(template: str, examples_path: str = "rag_documents") -> str:
    def read_example(example_name: str) -> str:
        example_dir = os.path.join(examples_path, example_name)
        try:
            with open(os.path.join(example_dir, "lore.txt"), "r", encoding="utf-8") as f:
                lore = f.read().strip()
            with open(os.path.join(example_dir, "summary.txt"), "r", encoding="utf-8") as f:
                summary = f.read().strip()
        except FileNotFoundError as e:
            raise ValueError(f"Missing file in {example_name}: {e.filename}")
        return f"Lore document:\n\n{lore}\n\nSummary:\n\n{summary}"

    example1_content = read_example("document1")

    filled_template = template.replace("{example}", example1_content)
    return filled_template 